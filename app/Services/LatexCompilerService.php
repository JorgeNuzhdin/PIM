...
class LatexCompilerService
{
    ...
    public function compile(string $texContent, array $images = []): array
    {
        ...
        // Incluye .sty/.tex (paquetes), .fd/.def (fuentes), .tfm/.pfb/.map/.cfg (métricas y mapas de fuentes), .pdf (recursos como all-twemojis.pdf)
        $packagesDir = resource_path('tex/packages');
        if (is_dir($packagesDir)) {
            foreach (glob($packagesDir . '/*.{sty,tex,fd,def,tfm,pfb,map,cfg,pdf}', GLOB_BRACE) as $pkgFile) {
