    ...
    public function index(Request $request)
{
    ...
    public function previewPdf(Request $request, $id)
    {
        ...
        $packagesRaw = $request->input('packages',     $problema->packages     ?? '');

        $packages  = \App\Services\LatexDocumentService::parsePackages($packagesRaw);
        $preambulo = \App\Services\LatexDocumentService::buildPreamble($packages, true);

        $body  = "\\exercise{" . \App\Services\LatexDocumentService::sanitizeTexForMacroArg($problemTex) . "}\n";
        if (trim($solutionTex) !== '') {
            $body .= "\n\\solution{" . \App\Services\LatexDocumentService::sanitizeTexForMacroArg($solutionTex) . "}\n";
        }

        $texContent = $preambulo . "\n\n\\begin{document}\n\n" . $body . "\n\\end{document}";
        $imageData  = \App\Services\LatexDocumentService::gatherImages($problemTex . ' ' . $solutionTex);
