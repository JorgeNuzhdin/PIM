...
@section('title', 'Problemas con errores')
...
<div class="container">
    <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:1.5rem; flex-wrap:wrap; gap:0.5rem;">
        <h1 style="margin:0; color:#2d3748;">⚠️ Problemas con errores reportados</h1>
        <a href="{{ route('problemas.index') }}" style="font-size:0.9rem; color:#4a5568; text-decoration:none;">← Volver al buscador</a>
    </div>

    @if($problemas->count() === 0)
        <div class="problema-card" style="text-align:center; color:#718096; border-left-color:#68d391;">
            ✅ No hay problemas con errores pendientes.
        </div>
    @else
    ...
    <div class="pagination-wrapper">
        @if ($problemas->hasPages())
            <div class="pagination">
                ...
                @endif
                @for ($p = 1; $p <= $problemas->lastPage(); $p++)
                    @if ($p == $problemas->currentPage())
                        <span class="page-item active">{{ $p }}</span>
                    @else
                        <a href="{{ $problemas->url($p) }}" class="page-item">{{ $p }}</a>
                    @endif
                @endfor
    ...
    @foreach($problemas as $problema)
        <div class="problema-card">
            <div class="problema-card-header">
                <h3>Problema #{{ $problema->id }}
                    @if($problema->title)
                        <span style="color:#718096; font-size:0.9rem; font-weight:400;"> — {{ $problema->title }}</span>
                    @endif
                </h3>
                <div style="display:flex; gap:0.5rem; align-items:center; flex-wrap:wrap;">
                    <button class="btn-report-index" onclick="openReportModal({{ $problema->id }})" title="Añadir otro reporte">
                        ⚠️ Reportar
                    </button>
                    <a href="{{ route('problemas.edit', ['id' => $problema->id]) }}" class="btn-edit-problema">
            ...
            @if($problema->tipoAgregado !== '000000000')
                <div class="tipo-badges">
                    ...
                    @foreach($tipoLabels as $bit => $label)
                        @if(($problema->tipoAgregado[$bit] ?? '0') === '1')
            ...
            @if($problema->tags && $problema->tags->count() > 0)
                <div style="margin-bottom:0.75rem; display:flex; flex-wrap:wrap; gap:0.4rem;">
                    @foreach($problema->tags as $tag)
                        <span style="display:inline-block; background:linear-gradient(135deg,#667eea,#764ba2); color:white; padding:0.2rem 0.6rem; border-radius:12px; font-size:0.8rem;">{{ $tag->tag }}</span>
                    @endforeach
                </div>
            @endif
            ...
            <table class="reportes-table">
                ...
                <tbody>
                    @foreach($problema->errorReports as $reporte)
                        <tr>
